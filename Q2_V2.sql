...
--the other method of solving this question
...
-- combine them based on their same column and find the channel's name have Marvel Entertainment and caluclate the Like_View_Ratio in video.
SELECT 
    v.title AS Video_Title, 
    c.channel_name AS Channel_Name, 
    v.likes / NULLIF(v.views, 0) AS Like_View_Ratio
FROM 
    video v,
    channel c,
    user u
WHERE 
    ...
    AND c.channel_name LIKE '%Marvel Entertainment%'
ORDER BY 
    v.title ASC;